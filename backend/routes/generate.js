...
import { generateImageFromPrompt } from '../../src/services/openaiService.js';
...
router.post('/generate', upload.single('image'), async (req, res) => {
  ...
  if (!prompt) {
    return res.status(400).json({
      error: 'Escreva um prompt para gerar a nova imagem.',
    });
  }
  ...
  try {
    const result = await generateImageFromPrompt(tempFilePath, prompt);
